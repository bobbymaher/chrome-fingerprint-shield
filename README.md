...
- Color-coded badges with category tags, target details, and inspectable JSON payloads.

### 2. Targeted AJAX Telemetry Endpoint Shield
- Intercepts `window.fetch()` and `XMLHttpRequest` calls targeting known event logging and telemetry paths (`/events`, `/telemetry`, `/analytics`, `/metrics`, `/perfMetrics`, `/log_event`, `/collect`, `/track`, `/beacon`, `/reporting`).
- Automatically mutes telemetry requests by returning a synthetic `200 OK` response without executing outbound network calls, leaving site functionality (`/graphql`, `/comments`, upvotes) 100% operational.
- Displayed under the 18th probe category (**AJAX Telemetry Path**) in the extension popup monitor.

### 3. Profile Switching & Dynamic Headers
...
### 8. Web Worker & ServiceWorker Isolation
...
### 9. Sub-Pixel DOMRect, SVGRect & Audio Noise
...
### 10. Smart WebRTC Private LAN IP Scrubbing
...
### 11. Multi-Monitor, Screen Details & Peripheral Neutering
...
### 12. Anti-Font Fingerprinting & OS Alignment
...
### 13. Same-Offset Timezone Anonymizer
...
- Anonymizes exact city location while maintaining zero offset difference to prevent timezone mismatch detection.

### 14. Google Privacy Sandbox Topics API Randomizer
